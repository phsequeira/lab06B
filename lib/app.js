...
app.get('/routes/:id', async(req, res) => {
  try {
    ...
    const data = await client.query('SELECT routes.id, routes.location, routes.route_name, routes.route_rating, routes.is_toprope, routes.owner_id  FROM routes JOIN locations ON routes.location_id = locations.id where routes.id=$1', [id]);
