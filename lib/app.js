...
app.get('/routes', async(req, res) => {
  try {
    const data = await client.query('SELECT * from routes');
...
app.get('/routes/:id', async(req, res) => {
  try {
    ...
    const data = await client.query('select * from routes where id=$1', [id]);
  ...
  app.put('/routes/:id', async(req, res) => {
    ...
    try {
      const data = await client.query(`
        UPDATE routes
        SET location = $1, route_name = $2, route_rating = $3, is_toprope = $4
        WHERE id=$5
        returning *;
      `,
